...
#
# 拆分说明：
#   state.py      — 全局状态管理（transfer_status, transfer_lock）
...
# ===== 从 state.py re-export（状态管理）=====
from state import transfer_status, transfer_lock, is_transfer_running

# ===== 从 pipeline.py re-export（转存执行）=====
...
from verify import (
    validate_share_link,
    update_expired_task,
    check_expired_tasks as _check_expired_tasks,
    fix_expired_tasks as _fix_expired_tasks,
...
def check_expired_tasks(limit=None):
    ...
    return _check_expired_tasks(transfer_status, limit)


def fix_expired_tasks():
    """薄包装：自动注入 transfer_status 和 transfer_lock"""
    return _fix_expired_tasks(transfer_status, transfer_lock)
